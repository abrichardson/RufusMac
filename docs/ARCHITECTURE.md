...
The Windows pipeline is now embedded from `Sources/MacusKit/Resources/windows-write.sh`. It checks prerequisites before erase, honors GPT/MBR, and verifies copies. Windows setup options are generated by `WindowsSetupOptions`, with account settings in the OEM/Panther tree. Windows writes stream output as the logged-in user and elevate only formatting. Multiboot is disabled. See the root README for current capabilities; the original architecture notes below describe the starting design.
...
## Testing
`Tests/MacusKitTests` (Swift Testing) covers `DiskParser` parsing and the
internal-disk rejection. The `macusctl` CLI doubles as a manual harness:
`macusctl list`, `macusctl preview <mode> [iso]`.
